        ...
        {Services.map((service: ServicesType, index: number) => (
          ...
          >
            ...
            >
              ...
              >
                {service.serviceName}
                <div className="absolute w-[140%] sm:w-[121%] text-[10px] sm:text-sm text-black">
                  <p className="absolute w-auto tracking-normal font-normal top-5 md:top-3 left-7 lg:top-4 lg:left-5 transform -translate-y-full bg-primary rounded-full px-2 py-1 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity duration-500 delay-[100ms]">
                    {service.hoverText1}
                  </p>
                  <p className="absolute w-auto tracking-normal font-normal left-[13rem] bottom-[1.5rem] sm:left-[30rem] sm:bottom-[4rem] md:bottom-[3rem] lg:bottom-20  md:left-[23rem] lg:left-[30rem] transform -translate-x-full -translate-y-1/2 bg-primary rounded-full px-2 py-1 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity duration-750 delay-[300ms]">
                    {service.hoverText2}
                  </p>
                  <p className="absolute w-auto tracking-normal font-normal bottom-[4rem] sm:bottom-[7rem]  md:bottom-[6rem] md:left-[6rem] left-[2rem] sm:left-[10rem] lg:left-48  lg:bottom-[8rem] transform translate-y-full bg-primary rounded-full px-2 py-1 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity duration-150 delay-[300ms]">
                    {service.hoverText3}
                  </p>
                  <p className={`absolute w-auto ${service.hoverText4 === "" ? "hidden bg-white" : ""} ${service.hoverText4 === "iconography" ? "left-4 bg-black" : ""} tracking-normal font-normal bottom-[-10px] sm:left-[20rem] lg:bottom-[1rem] md:bottom-1/2 left-20 md:left-40 lg:left-[15rem] transform translate-x-full translate-y-1/2 bg-primary rounded-full px-2 py-1 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity duration-1000 delay-[400ms]`}>
                    {service.hoverText4}
                  </p>
                </div>
              </h1>
